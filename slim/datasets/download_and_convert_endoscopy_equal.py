...
def _get_filenames_and_classes(dataset_dir):
  ...
  for filename in os.listdir(endoscopy_root):
    path = os.path.join(endoscopy_root, filename)
...
def _get_dataset_filename(dataset_dir, cross_val_num, split_name, shard_id):
  output_filename = 'cls_data_%s_equal_%d_%s_%05d-of-%05d.tfrecord' % (
...
def run(dataset_dir):
  ...
  filenames_per_classes = []
  for i, class_name in enumerate(class_names):
    filenames_per_classes.append([photo for photo in photo_filenames \
                        if os.path.basename(os.path.dirname(photo)) == class_name])
  min_num_photos = min([len(x) for x in filenames_per_classes])
  
  for filenames_per_class in filenames_per_classes:
    num_validation = round(min_num_photos*_VALIDATION_RATIO)
    for i in range(_NUM_CROSS_VAL):
      random.shuffle(filenames_per_class)
      validation_filenames[i] += filenames_per_class[:num_validation]
      training_filenames[i] += filenames_per_class[num_validation:min_num_photos]
  ...
  for i in range(_NUM_CROSS_VAL):
    _convert_dataset('train', training_filenames[i], class_names_to_ids,
                    dataset_dir, i)
    _convert_dataset('validation', validation_filenames[i], class_names_to_ids,
                    dataset_dir, i)
